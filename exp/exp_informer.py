...
class Exp_Informer(Exp_Basic):
    ...
    def _select_optimizer(self):
        ...
        ), lr=self.args.learning_rate, weight_decay=self.args.weight_decayy)
        return model_optim
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
            with torch.no_grad():
                ...
                loss = criterion(pred.detach().cpu(),
                                 true.detach().cpu()).item()
            total_loss.append(loss)
        total_loss = np.average(total_loss)
        self.model.train()
        return total_loss

    def train(self, setting, only_val=False):
        epochs_time = []
        ...
        if not only_val:
            ...
            self.best_val_score = early_stopping.best_score
        return self.model
    ...
    def test(self, setting):
        ...
        with open(folder_path+"/inferences_time.txt", "w") as file:
            file.write(
                f"{statistics.mean(inferences_time)},{statistics.stdev(inferences_time)}")
        return
